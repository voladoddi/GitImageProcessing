function [Out_Image, Labels ,List , Count_List] = Connected_Components_8_Connectivity( In_Image,gray_level_value)
clc
...
V = gray_level_value; %%% The pixels with value V will only be marked
                    ...
                    if( ( In_Image(i,j) == In_Image(i - 1,j) ) )
                     Label_Image(i,j) = Label_Image(i - 1,j);
                        ...
                        %%%%%% Doubles %%%%%%%
                         ...
                         if(( In_Image(i,j) == V ) && ( In_Image(i,j) ~= In_Image(i, j -1) )  && ( In_Image(i,j) ~= In_Image(i - 1, j - 1) ) && (In_Image(i,j) ~= In_Image(i - 1, j )) && (In_Image(i,j) ~= In_Image(i - 1, j + 1)))
                             L = L + 1;
                             Label_Image(i,j) = gray_level_value;
                         end
